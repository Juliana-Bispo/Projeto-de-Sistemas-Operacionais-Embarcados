...
struct GerenciadorPrateleira {
    Rect prateleiraArea; // A única área grande que contém todas as latas
    int capacidadeTotal;
    map<string, int> contagemMarcas;
    mutable mutex mtx;

    GerenciadorPrateleira() {
        // ATENÇÃO: Calibre este retângulo para cobrir a área onde as 4 latas ficarão
        prateleiraArea = Rect(40, 50, 250, 150); // (x, y, largura_total, altura_total)
        capacidadeTotal = 4;
        
        // Marcas que o sistema conhece
        contagemMarcas["Guaraná"] = 0;
    ...
    // Atualiza a contagem com base em um novo mapa de detecções
    void atualizarContagem(const map<string, int>& novasDeteccoes) {
        lock_guard<mutex> lock(mtx);
        // Reseta a contagem atual
...
string detectarCorPredominante(const Mat& bgr_roi) {
    ...
    // --- FAIXAS DE COR AJUSTADAS ---

    // Vermelho (Coca-Cola): Geralmente robusto
    ...
    // Verde (Guaraná): Intervalo de Matiz (H) ampliado e Saturação/Brilho (S/V) mínimos reduzidos
    Mat maskVerde;
    inRange(hsv, Scalar(35, 40, 40), Scalar(85, 255, 255), maskVerde);
    contadorDePixels["Verde"] = countNonZero(maskVerde);

    // Azul (Pepsi): Intervalo de Matiz (H) ampliado e Saturação/Brilho (S/V) mínimos bem reduzidos para capturar o azul escuro
    Mat maskAzul;
    inRange(hsv, Scalar(95, 60, 40), Scalar(130, 255, 255), maskAzul);
    contadorDePixels["Azul"] = countNonZero(maskAzul);
    
    // Laranja (Fanta): Geralmente robusto
    Mat maskLaranja;
    ...
    // Limiar mínimo para considerar uma detecção válida
...
// A struct de resultado agora contém um mapa de contagem
...
void threadDetecao(GerenciadorPrateleira& prateleira) {
    ...
    while (!shouldStop) {
        ...
        // Desenha a área da prateleira para visualização
...
int main() {
    ...
    while (true) {
        ...
        if (resultQueue.try_pop(result)) {
            // A lógica principal agora atualiza o mapa de contagem
