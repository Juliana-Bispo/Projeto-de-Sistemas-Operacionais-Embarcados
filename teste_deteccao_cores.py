...
def cor_dominante_h(roi_hsv):
    h, _, _ = cv2.split(roi_hsv)
    hist = cv2.calcHist([h], [0], None, [180], [0, 180])
    return int(np.argmax(hist))

cores_refri = {
    'Coca-Cola': [(0, 10), (170, 179)],
    'Sukita': [(10, 30)],
    'Guaraná': [(35, 85)],
    'Pepsi': [(90, 135)],
    'Fanta Uva': [(130, 160)]
...
# Carregar imagem
img = cv2.imread('imagem_real_refri.jpg')
if img is None:
    ...
    if area > 1000 and h > 50 and w > 50:
        ...
        for nome, faixas in cores_refri.items():
            for (hmin, hmax) in faixas:
                if hmin <= hue <= hmax:
                    cv2.rectangle(output, (x, y), (x+w, y+h), (0, 255, 0), 2)
                    cv2.putText(output, f"{nome} ({hue})", (x, y-10),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
                    print(f"Lata identificada: {nome} (Hue: {hue})")
                    identificado = True
                    break
            if identificado:
                break
        if not identificado:
            cv2.rectangle(output, (x, y), (x+w, y+h), (0, 0, 255), 2)
            cv2.putText(output, f"Desconhecido ({hue})", (x, y-10),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1)
            print(f"Lata {i} com Hue {hue} não identificada.")
